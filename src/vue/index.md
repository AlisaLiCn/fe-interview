...
### computed和watch区别
定义：
- computed: 用于计算属性（Computed Properties），它会基于依赖的数据变化自动缓存结果。只有当依赖的数据发生变化时，computed属性才会重新计算。这适用于需要根据响应式数据动态计算值的情况，且结果需要缓存的场景，比如复杂计算或需要对多次访问进行性能优化。
- watch: 用于侦听属性（Watchers），可以在数据变化时执行特定的回调函数。watch适用于监听数据变化并执行异步操作、进行数据深度观察、或在数据变化时执行复杂逻辑的场景，比如监听表单输入、调用API请求等。

适用场景：
- 使用computed：当需要基于一个或多个响应式数据的变化来计算一个值时，并且该计算结果可以被缓存。
- 使用watch：当需要在数据变化时执行异步或开销较大的操作（有副作用）时。
...
- 免去了重复的创建节点，大型应用会受益于这个改动，优化运行时的内存占用。
...
但是这样做有以下问题：
- 添加或删除对象的属性时，Vue检测不到。因为添加或删除的对象没有在初始化进行响应式处理 ，只能通过$set来调用Object.defineProperty()处理。
- 无法监控到数组下标和长度的变化。
...
当页面的状态发生改变，需要对页面的DOM结构进行调整的时候，
- 首先根据变更的状态，重新构建起一棵对象树，
- 然后将这棵新的对象树和旧的对象树进行比较，记录下两棵树的差异。
...
- key与v-for结合使用。用v-for更新已渲染过的元素列表时，由于没有key时默认使用”就地复用”的策略。如果数据项的顺序发生了改变，Vue不会移动DOM元素来匹配数据项的顺序，而是简单复用此处的每个元素。因此通过为每个列表项提供一个key值，来以便Vue跟踪元素的身份，从而高效的实现复用。这个时候key的作用是为了高效的更新渲染虚拟DOM。
